(function($, Models, Collections, Views) {
    $(document).ready(function() {
		 ...
		 * Bulletin collection
		 */
		/*Collections.Bulletins = Backbone.Collection.extend({
			model: Models.Bulletin,
			action: 'ae-fetch-bulletins',
			initialize: function() {
				this.paged = 1;
			}
		});*/
		/**
		 * Define bulletin item view
		 */
		/*BulletinItem = Views.PostItem.extend({
			tagName: 'li',
			className: 'bulletin-item',
			template: _.template($('#ae-bulletin-loop').html()),
			onItemBeforeRender: function() {
				//before render item
			},
			onItemRendered: function() {
		...
		/**
		 * List view control bulletins list
		 */
		/*ListBulletins = Views.ListPost.extend({
			tagName: 'ul',
			itemView: BulletinItem,
			itemClass: 'bulletin-item'
		});*/
